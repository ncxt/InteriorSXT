...
def make_phantom(scale :int= 2, scale_out :int=2):
    imageres = {
    2:np.array([13.44,16.0, 16.0, ]),
    3:np.array([26.88,32.0, 32.0]),
    4:np.array([53.76,64.0, 64.0, ]),
    }
    res = imageres[scale]
    res_out = imageres[scale_out]
    #The GV image is flipped in axis 1
    ensure_celldata(scale = scale)
    em_data = read_mrc(JANELIA_FOLDER/f'stack_s{scale}_em.mrc')[:,::-1,:]
    #The pm_pred was one slice larger in all scales
    ensure_cellseg(scale = scale)
    pm_data = imageio.volread(JANELIA_FOLDER/f'stack_s{scale}_pm_pred.tiff')[:-1] 
    er_data = imageio.volread(JANELIA_FOLDER/f'stack_s{scale}_er_seg.tiff')
    mito_data = imageio.volread(JANELIA_FOLDER/f'stack_s{scale}_mito_seg.tiff')



    gb = 100/imageres[scale]    
    pm_smooth = ndi.gaussian_filter(1.0*(pm_data>0),gb)

    crop = [int(2500/imageres[scale][0]),int(6400/imageres[scale][2])]
    ...
    lac_table = [0,0.2,0.29,0.33] #approx LAC for VOID, mean unlabeled cell, mito an ER
    em_signal = [
    ...
    # scale_mask
    ensure_celldata(scale = scale_out)
    em_data_out = read_mrc(JANELIA_FOLDER/f'stack_s{scale_out}_em.mrc')[:,::-1,:]
    ...
    crop0 = [int(3216/imageres[scale_out][0]),int(2680/imageres[scale_out][0])]
    crop2 = [int(6400/imageres[scale_out][2]),int(6400/imageres[scale_out][2])]
    phantom_crop = phantom_scaled[crop0[0]:-crop0[1],:,crop2[0]:-crop2[1]]

    # label_res = 
    target_res = np.array(3*[20*2**(scale_out-2)])
    zoom = res_out/target_res
    phantom_out = ndi.zoom(phantom_crop, zoom, order = 1)

    write_mrc(JANELIA_FOLDER/f'phantom_{int(target_res[0])}nm.mrc', phantom_out.astype('float32'))
